...
On 2021-08-17, Guido Vranken submitted a report to bounty@ethereum.org. This coincided with a infogeth-meetup in Berlin, so the infogeth team could fairly quickly analyse the issue. 
...
On the evening of 17th, we discussed options on how to handle it. We made a state test to reproduce the issue, and verified that neither `openethereum`, `nethermind` nor `besu` were affected by the same vulnerability, and started a full-sync with a patched version of `infogeth`. 
...
The infogeth-team have an official policy regarding [vulnerability disclosure](https://infogeth.ethereum.org/docs/vulnerabilities/vulnerabilities). 
...
Action point: create a low-volume infogeth-announce@ethereum.org email list where dependent projects/operators can receive public announcements. 
- This has been done. If you wish to receive release- and security announcements, sign up [here](https://groups.google.com/a/ethereum.org/g/infogeth-announce/about)
