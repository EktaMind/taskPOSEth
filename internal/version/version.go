...
func versionInfo(info *debug.BuildInfo) string {
	// If the main package is from our repo, prefix version with "infogeth".
	if strings.HasPrefix(info.Path, ourPath) {
		return fmt.Sprintf("infogeth %s", info.Main.Version)
	}
	...
	if mod == nil {
		...
		// version.
		return version + fmt.Sprintf("infogeth %s", params.VersionWithMeta)
	}
