...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of infogeth is "infogeth". If no
...
func (c *Config) NodeName() string {
	...
	if name == "infogeth" || name == "infogeth-testnet" {
		name = "Geth"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "infogeth" instances.
...
func (c *Config) ResolvePath(path string) string {
	...
	// by infogeth 1.4 are used if they exist.
	if warn, isOld := isOldGethResource[path]; isOld {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn && !c.oldGethResourceWarning {
				...
				log.Warn("Using deprecated resource file, please move this file to the 'infogeth' subdirectory of datadir.", "file", oldpath)
			}
			return oldpath
		}
	}
