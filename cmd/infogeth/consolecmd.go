...
See https://infogeth.ethereum.org/docs/interface/javascript-console.`,
	}
...
which exposes a node admin interface as well as the Ðapp JavaScript API.
See https://infogeth.ethereum.org/docs/interface/javascript-console.
This command allows to open a console on a running infogeth node.`,
	}
...
JavaScript API. See https://infogeth.ethereum.org/docs/interface/javascript-console`,
	}
)

// localConsole starts a new infogeth node, attaching a JavaScript console to it at the
// same time.
func localConsole(ctx *cli.Context) error {
	...
	if err != nil {
		return fmt.Errorf("Failed to attach to the inproc infogeth: %v", err)
	}
...
// remoteConsole will connect to a remote infogeth instance, attaching a JavaScript
// console to it.
func remoteConsole(ctx *cli.Context) error {
	...
	if err != nil {
		utils.Fatalf("Unable to attach to remote infogeth: %v", err)
	}
	config := console.Config{
		DataDir: utils.MakeDataDir(ctx),
		DocRoot: ctx.String(utils.JSpathFlag.Name),
...
// ephemeralConsole starts a new infogeth node, attaches an ephemeral JavaScript
...
infogeth --exec "%s" console`, b.String())
	return nil
}
