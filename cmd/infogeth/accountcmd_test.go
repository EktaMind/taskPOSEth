...
func TestAccountListEmpty(t *testing.T) {
	infogeth := runGeth(t, "account", "list")
	infogeth.ExpectExit()
}
...
`
	...
	{
		infogeth := runGeth(t, "account", "list", "--datadir", datadir)
		infogeth.Expect(want)
		infogeth.ExpectExit()
	}
	{
		infogeth := runGeth(t, "--datadir", datadir, "account", "list")
		infogeth.Expect(want)
		infogeth.ExpectExit()
	}
}
...
func TestAccountNew(t *testing.T) {
	infogeth := runGeth(t, "account", "new", "--lightkdf")
	defer infogeth.ExpectExit()
	infogeth.Expect(`
...
func TestAccountHelp(t *testing.T) {
	infogeth := runGeth(t, "account", "-h")
	infogeth.WaitExit()
	if have, want := infogeth.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}

	infogeth = runGeth(t, "account", "import", "-h")
	infogeth.WaitExit()
	if have, want := infogeth.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	infogeth := runGeth(t, "--lightkdf", "account", "import", "-password", passwordFile, keyfile)
	defer infogeth.ExpectExit()
	infogeth.Expect(expected)
}
...
func TestAccountUpdate(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	infogeth := runGeth(t, "account", "update",
...
func TestWalletImport(t *testing.T) {
	infogeth := runGeth(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer infogeth.ExpectExit()
	infogeth.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := os.ReadDir(filepath.Join(infogeth.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	infogeth := runMinimalGeth(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(infogeth.StderrText(), m) {
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	infogeth.SetTemplateFunc("keypath", func(file string) string {
