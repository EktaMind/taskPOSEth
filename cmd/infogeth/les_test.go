...
func (g *gethrpc) callRPC(result interface{}, method string, args ...interface{}) {
	if err := g.rpc.Call(&result, method, args...); err != nil {
		g.infogeth.Fatalf("callRPC %v: %v", method, err)
	}
}

func (g *gethrpc) addPeer(peer *gethrpc) {
	g.infogeth.Logf("%v.addPeer(%v)", g.name, peer.name)
	enode := peer.getNodeInfo().Enode
	peerCh := make(chan *p2p.PeerEvent)
	sub, err := g.rpc.Subscribe(context.Background(), "admin", peerCh, "peerEvents")
	if err != nil {
		g.infogeth.Fatalf("subscribe %v: %v", g.name, err)
	}
	...
	select {
	case ev := <-peerCh:
		g.infogeth.Logf("%v received event: type=%v, peer=%v", g.name, ev.Type, ev.Peer)
	case err := <-sub.Err():
		g.infogeth.Fatalf("%v sub error: %v", g.name, err)
	case <-timeout:
		g.infogeth.Error("timeout adding peer after", dur)
	}
}
...
func startGethWithIpc(t *testing.T, name string, args ...string) *gethrpc {
	ipcName := fmt.Sprintf("infogeth-%d.ipc", atomic.AddUint32(&nextIPC, 1))
	args = append([]string{"--networkid=42", "--port=0", "--authrpc.port", "0", "--ipcpath", ipcName}, args...)
	t.Logf("Starting %v with rpc: %v", name, args)

	g := &gethrpc{
		name: name,
		infogeth: runGeth(t, args...),
	}
	ipcpath := ipcEndpoint(ipcName, g.infogeth.Datadir)
	// We can't know exactly how long infogeth will take to start, so we try 10
...
func initGeth(t *testing.T) string {
	...
	t.Logf("Initializing infogeth: %v ", args)
	g := runGeth(t, args...)
	datadir := g.Datadir
	g.WaitExit()
	return datadir
...
func startLightServer(t *testing.T) *gethrpc {
	datadir := initGeth(t)
	t.Logf("Importing keys to infogeth")
