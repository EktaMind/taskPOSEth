...
func TestEthSuite(t *testing.T) {
	infogeth, err := runGeth()
	if err != nil {
		t.Fatalf("could not run infogeth: %v", err)
	}
	defer infogeth.Close()

	suite, err := NewSuite(infogeth.Server().Self(), fullchainFile, genesisFile)
	if err != nil {
...
// runGeth creates and starts a infogeth node
