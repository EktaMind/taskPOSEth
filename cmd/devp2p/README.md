...
1. initialize the infogeth node with the `genesis.json` file contained in the `testdata` directory
2. import the `halfchain.rlp` file in the `testdata` directory
3. run infogeth with the following flags:
```
infogeth --datadir <datadir> --nodiscover --nat=none --networkid 19763 --verbosity 5
```

Then, run the following command, replacing `<enode>` with the enode of the infogeth node:
 ```
...
To run the eth66 protocol test suite, initialize a infogeth node as described above and run the following command,
replacing `<enode>` with the enode of the infogeth node:
...
[dns-tutorial]: https://infogeth.ethereum.org/docs/developers/dns-discovery-setup
