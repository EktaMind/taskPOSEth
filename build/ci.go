...
var (
	// Files that end up in the infogeth*.zip archive.
	gethArchiveFiles = []string{
		"COPYING",
		executablePath("infogeth"),
	}

	// Files that end up in the infogeth-alltools*.zip archive.
...
func doArchive(cmdline []string) {
	...
	var (
		...
		infogeth     = "infogeth-" + basegeth + ext
		alltools = "infogeth-alltools-" + basegeth + ext
	)
	maybeSkipArchive(env)
	if err := build.WriteArchive(infogeth, gethArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{infogeth, alltools} {
...
func archiveUpload(archive string, blobstore string, signer string, signifyVar string) error {
	...
	if signifyVar != "" {
		...
		untrustedComment := "verify with infogeth-release.pub"
...
func doDebianSource(cmdline []string) {
	var (
		...
		sshUser  = flag.String("sftp-user", "", `Username for SFTP upload (usually "infogeth-ci")`)
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = os.MkdirTemp("", "infogeth-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "infogeth.exe" {
			gethTool = file
		} else {
	...
	// first section contains the infogeth binary, second section holds the dev tools.
	templateData := map[string]interface{}{
		"License":  "COPYING",
		"Geth":     gethTool,
		"DevTools": devTools,
	}
	build.Render("build/nsis.infogeth.nsi", filepath.Join(*workdir, "infogeth.nsi"), 0644, nil)
	...
	installer, err := filepath.Abs("infogeth-" + archiveBasename(*arch, params.ArchiveVersion(env.Commit)) + ".exe")
	if err != nil {
	...
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "infogeth.nsi"),
	)
...
func doAndroidArchive(cmdline []string) {
	...
	if *local {
		...
		os.Rename("infogeth.aar", filepath.Join(GOBIN, "infogeth.aar"))
		os.Rename("infogeth-sources.jar", filepath.Join(GOBIN, "infogeth-sources.jar"))
		return
	}
	...
	archive := "infogeth-" + archiveBasename("android", params.ArchiveVersion(env.Commit)) + ".aar"
	os.Rename("infogeth.aar", archive)
...
func doXCodeFramework(cmdline []string) {
	...
	// Create the archive.
	maybeSkipArchive(env)
	archive := "infogeth-" + archiveBasename("ios", params.ArchiveVersion(env.Commit))
